...
# VEXLearn-Template
VEXLearn is not a competition-focused template. It is a **teaching template**, its goal is to help users understand how VEX software works from the ground up. Every file in the template is documented with introductions, structure explanations, inline comments, and troubleshooting Q&As so that users can learn by reading and modifying real code. The template and the guide has been designed with people with no prior coding experience in mind. The PDF file covers the basic fundemental syntax users need to know about C++, tips for VEX coders that covers autonomous code writing and team dynamics, and concepts essential to writing VEX software such as object oriented programming and Proportional–Integral–Derivative Controller (PID).
